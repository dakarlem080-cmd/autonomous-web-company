...
export default function Layout({children}:{children:React.ReactNode}){
 useEffect(()=>{
  ...
  const items=Array.from(document.querySelectorAll<HTMLElement>(".navItem"));items.forEach(item=>{const label=(item.querySelector("span")?.textContent||item.textContent||"").trim();const href=routes[label];if(!href)return;if(item.tagName==="A")item.setAttribute("href",href);else item.onclick=()=>{window.location.href=href}});
  return()=>items.forEach(item=>{item.onclick=null});
 },[]);
 return <html><head><title>Autonomous Web Company</title></head><body><AuthBootstrap/>{children}</body></html>
}
