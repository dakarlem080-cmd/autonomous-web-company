...
class Employee(Base):
    ...
    id:Mapped[int]=mapped_column(primary_key=True); project_id:Mapped[int]=mapped_column(ForeignKey("projects.id")); name:Mapped[str]=mapped_column(String(120)); role:Mapped[str]=mapped_column(String(120)); agent:Mapped[str]=mapped_column(String(120),default=""); instructions:Mapped[str]=mapped_column(Text,default=""); objectives:Mapped[list]=mapped_column(JSON,default=list); tools:Mapped[list]=mapped_column(JSON,default=list); permissions:Mapped[list]=mapped_column(JSON,default=list); model_id:Mapped[int|None]=mapped_column(ForeignKey("ai_models.id"),nullable=True); autonomy_level:Mapped[str]=mapped_column(String(40),default="execute"); active:Mapped[bool]=mapped_column(Boolean,default=True); created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=now)
class AIModel(Base):
