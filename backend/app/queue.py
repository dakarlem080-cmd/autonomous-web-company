...
QUEUE_KEY="awc:jobs";DLQ_KEY="awc:jobs:dead"
def client()->Redis:
...
async def retry(job:dict,error:str):
    job["attempts"]+=1;job["error"]=error[:4000]
    if job["attempts"]>=job["max_attempts"]:return await dead_letter(job,error)
    await asyncio.sleep(min(60,2**job["attempts"]));r=client();await r.rpush(QUEUE_KEY,json.dumps(job));await r.aclose()
