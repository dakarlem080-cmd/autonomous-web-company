...
def client() -> Redis:
    if not settings().REDIS_URL:
        raise RuntimeError("REDIS_URL is required for queued jobs")
...
async def reserve(timeout: int = 5):
    r = client()
    item = await r.blpop(QUEUE_KEY, timeout=timeout)
    await r.aclose()
...
async def retry(job: dict, error: str):
    ...
    if job["attempts"] >= job["max_attempts"]:
        return await dead_letter(job, error)

    await asyncio.sleep(min(60, 2**job["attempts"]))
    r = client()
    await r.rpush(QUEUE_KEY, json.dumps(job))
    await r.aclose()
