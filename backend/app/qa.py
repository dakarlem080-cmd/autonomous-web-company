...
def static_checks(root:Path)->dict:
    ...
    if layout.exists():checks["canonical"]="canonical" in layout.read_text(encoding="utf-8",errors="replace")
    return checks

def run_cmd(cmd:list[str],cwd:Path,timeout:int=300):
    try:
        p=subprocess.run(cmd,cwd=cwd,text=True,capture_output=True,timeout=timeout);return p.returncode,(p.stdout+p.stderr)[-12000:]
...
def run_qa_sync(root:str,smoke_url:str|None=None)->QAResult:
    ...
    if not path.exists():return QAResult(passed=False,errors=["workspace_missing"])
    checks.update(static_checks(path));executed += list(checks)
    pkg=path/"package.json"
    if pkg.exists():
        try:
            data=json.loads(pkg.read_text());scripts=data.get("scripts",{})
            if "build" in scripts:
                code,out=run_cmd(["npm","run","build"],path);executed.append("npm_build");checks["build"]=code==0
                if code!=0:errors.append("npm_build_failed:"+out[-2000:])
            if "test" in scripts:
                code,out=run_cmd(["npm","test","--","--runInBand"],path);executed.append("npm_test");checks["tests"]=code==0
    ...
    checks["basic_secret_scan"]=not any(x in source for x in ["-----BEGIN PRIVATE KEY-----","sk_live_"]);executed.append("basic_secret_scan")
    if not checks["basic_secret_scan"]:errors.append("potential_secret_detected")
    if smoke_url:
        try:
            r=httpx.get(smoke_url,timeout=20,follow_redirects=True);checks["http_smoke"]=200<=r.status_code<400;executed.append("http_smoke")
    ...
    return QAResult(passed=not errors and all(v is not False for v in checks.values()),checks=checks,errors=errors,warnings=warnings,executed_checks=executed)
