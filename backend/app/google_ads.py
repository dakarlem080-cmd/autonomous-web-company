...
class GoogleAds:
    def __init__(self,oauth:dict|None=None,customer_id:str|None=None):
        self.oauth=oauth or {};self.customer_id=(customer_id or _cfg("GOOGLE_ADS_CUSTOMER_ID")).replace("-","");self.login_customer_id=_cfg("GOOGLE_ADS_LOGIN_CUSTOMER_ID").replace("-","");self.developer_token=_cfg("GOOGLE_ADS_DEVELOPER_TOKEN")
    def ready(self):return bool(self.oauth.get("access_token") and self.developer_token)
    def _headers(self):
        c=credentials(self.oauth)
        if c.expired and c.refresh_token:c.refresh(request=Request())
        headers={"Authorization":f"Bearer {c.token}","developer-token":self.developer_token,"Content-Type":"application/json"}
    ...
    async def accessible_customers(self):
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            return {"connected":True,"customer_ids":[x.rsplit("/",1)[-1] for x in r.json().get("resourceNames",[])]}
    async def campaign_metrics(self,customer_id:str|None=None):
        cid=(customer_id or self.customer_id).replace("-","")
        if not cid:return {"connected":False,"reason":"customer_id_not_configured"}
        if not self.ready():return {"connected":False,"reason":"oauth_or_developer_token_not_configured"}
        query="SELECT campaign.id, campaign.name, campaign.status, metrics.impressions, metrics.clicks, metrics.cost_micros, metrics.conversions FROM campaign WHERE segments.date DURING LAST_30_DAYS ORDER BY metrics.impressions DESC"
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            for batch in r.json():
                for row in batch.get("results",[]):
                    ...
                    rows.append({"id":campaign.get("id"),"name":campaign.get("name"),"status":campaign.get("status"),"impressions":int(metrics.get("impressions",0)),"clicks":int(metrics.get("clicks",0)),"cost":float(metrics.get("costMicros",0))/1000000,"conversions":float(metrics.get("conversions",0))})
