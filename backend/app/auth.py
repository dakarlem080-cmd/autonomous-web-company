...
from app.models import Membership, Organization, SessionToken, User
...
async def create_user(
    ...
    if len(password) < 12:
        raise ValueError("password_min_length_12")
    ...
    organization = Organization(
        name=(name.strip() or email.split("@")[0])[:200],
        slug=slugify(f"{name}-{user.id}"),
    )
    s.add(organization)
    await s.flush()
    s.add(
...
async def get_identity(s: AsyncSession, token: str | None):
    ...
    if not row or row.expires_at <= datetime.now(timezone.utc):
        return None
    user = await s.scalar(select(User).where(User.id == row.user_id, User.active))
    if not user:
        return None
    memberships = (
...
async def has_project_access(
    ...
    return bool(
        membership and ROLE_LEVEL.get(membership.role, 0) >= ROLE_LEVEL[minimum]
    )
