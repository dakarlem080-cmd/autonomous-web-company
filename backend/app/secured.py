...
from app.models import Project, User, Membership, SessionToken, AuditLog
...
class SecurityMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if path in PUBLIC_EXACT or any(path.startswith(p) for p in PUBLIC_PREFIX): return await call_next(request)
        ...
        async with Session() as s:
            ...
            if path=="/api/projects" and request.method=="GET":
                ...
                return JSONResponse([{"id":x.id,"name":x.name,"domain":x.domain,"repo":x.repo,"branch":x.branch,"goal":x.goal,"language":x.language,"dry_run":x.dry_run,"active":x.active} for x in rows])
            ...
            if match:
                ...
                if request.method in {"POST","PUT","PATCH","DELETE"} and ROLE_LEVEL.get(membership.role,0)<ROLE_LEVEL["Member"]:return JSONResponse({"detail":"insufficient_role"},status_code=403)
